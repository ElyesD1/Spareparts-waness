...
class SaleItemService {
  static final String _baseUrl = AppConfig.saleItemUrl;
  ...
  Future<SaleItem> addSaleItem(SaleItem saleItem) async {
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to add sale item: ${response.statusCode} - ${response.body}',
      );
    }
  }
  ...
  Future<SaleItem> updateSaleItem(String id, SaleItem saleItem) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to update sale item: ${response.statusCode} - ${response.body}',
      );
    }
  }
  ...
  Future<void> deleteSaleItem(String id) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 204) {
      throw Exception(
        'Failed to delete sale item: ${response.statusCode} - ${response.body}',
      );
    }
  }
  ...
  Future<List<SaleItem>> getAllSaleItems() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to load all sale items: ${response.statusCode} - ${response.body}',
      );
    }
  }
  ...
  Future<SaleItem?> getSaleItemById(String id) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        'Failed to load sale item: ${response.statusCode} - ${response.body}',
      );
    }
  }
}
